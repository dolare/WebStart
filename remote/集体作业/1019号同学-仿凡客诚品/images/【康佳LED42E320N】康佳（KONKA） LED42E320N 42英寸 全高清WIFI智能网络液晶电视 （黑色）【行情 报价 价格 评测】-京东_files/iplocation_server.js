...
try{if(location.href.indexOf("localtest=true")>0){stockServiceDomain = "http://webstock.jd.com";rmsurl="http://rms.shop.360buy.net";}}catch(e){}
//cookie operate
				...
				if(pageConfig.product.isFlashPurchase){
					...
					if(r.type==0){
						if(pageConfig.product.isLOC){
							$("<div class='dt'>服\u3000\u3000务：</div><div class='dd'><a class='shouhoudaojia' href='#none'></a>由<a href='"+$.getShopUrl(r)+"' target='_blank' clstag='shangpin|keycount|product|bbtn' class='hl_red'>"+r.vender+"</a>提供服务和售后"
...
function getBuyUrl(skuId){
    ...
    if(!count)count=1;
	if(pageConfig.product.isLOC){
		var flag=location.href.indexOf("loctest=true")>0;
		if(flag) return "http://cart.jd.com/cart/dynamic/gateForSubFlow.action?wids="+skuId+"&nums="+count+"&subType=22";
	}
...
function SetNotifyByNoneStock(stockstatus,ext) {
    ...
    if (stockstatus!=34&&stockstatus!=0&&warestatus==1&&pageConfig.product.skuid!=938747&&pageConfig.product.skuid!=938749){
		...
		/**
		 * @换原分期付款Icon;弹层加白条展现逻辑
		 * @by UED liuwei1 in 2014-5-29
		 * @PM 商城:容珏/金融:苏霓
		 */
        //var choose_btn_divide_a = $('#choose-btn-divide a');
        //choose_btn_divide_a.attr('clstag','shangpin|keycount|product|baitiao').css({
            //width:100
        //});
        //choose_btn_divide_a.find('b').css({
            //background: 'url(http://img10.360buyimg.com/da/jfs/t187/274/284523263/8856/bceb4ca8/5385bac9Nbc69d7f7.png) no-repeat'
        //});
...
function newEasyBuyInit(){
	if(dCashDescInfo.loadPriceCnt==dCashDescInfo.loadStockCnt){
		if(itemEasyBuy.bigger10&&pageConfig.product.isStock&&!pageConfig.product.YuShou
			&&pageConfig.product.cat[1]!=794&&pageConfig.product.cat[1]!=6880&&pageConfig.product.cat[1]!=703&&pageConfig.product.cat[0]!=4938&&pageConfig.product.cat[2]!=6980&&!pageConfig.product.isHeyue&&!pageConfig.product.isLOC){
			...
			/*if(useNewEasybuy){
				$('#choose-btn-easybuy .btn-easybuy').jdThickBox({
					type: 'iframe',
					source: 'http://easybuy.jd.com/skuDetail/showEasybuyList.action?skuId='+pageConfig.product.skuid+'&num='+$("#buy-num").val()+'&rid='+new Date(),
					width: 400,
					height: 180,
					title: '轻松购',
					_box: "box的id",
...
function getTJAreaSku(r){
    ...
    if(r&&r.resultCode==1&&r.skuId&&r.skuId!=pageConfig.product.skuid+''){
        ...
        //window.location="http://item.jd.com/"+r.skuId+".html";
    }
}
//获取配送库存信息或下一级地址
